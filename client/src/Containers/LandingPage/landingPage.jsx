...
const LandingPage = ({ authenticate }) => {
  ...
  const SubmitNewPassword = (event) => {
    ...
    if (
      new_password_password === new_password_confirm &&
      new_password_password.length > 7
    ) {
    ...
    if (signin_username.length > 3 && signin_password.length > 7) {
      ...
      if (number_regex.exec(value) !== null) {
        ...
        axios.post(`http://localhost:8000/login`, context).then((response) => {
          ...
          if (
            JSON.stringify(data) === JSON.stringify(username_err) ||
            JSON.stringify(data) === JSON.stringify(password_err)
          ) {
      ...
      if (signin_username.length < 4) {
        dummy.push({
          error_type: "Username",
          message: "The username should be atleast 4 characters",
        });
      }
      if (signin_password.length < 8) {
        dummy.push({
          error_type: "Password",
          message:
            "Password must contain number and should be atleast 8 characters",
        });
      }
  ...
  const SignupCredentialSubmitHandler = (event) => {
    event.preventDefault();
    if (
      signup_username.length > 3 &&
      signup_password.length >= 8 &&
      signup_email.length >= 11 &&
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <main
          className="landingpage-middle-flex"
          style={
            login_card ||
            signup_card ||
            forget_password_card ||
        ...
        >
          ...
          <button id="middle-flex-btn" onClick={SignupCardHandler}>
            CREATE ACCOUNT
          </button>
        </main>
      </main>
